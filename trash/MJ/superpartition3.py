...
#this takes in a sequence of integers for example [3,2,-3] and returns all the tableaux resulting from e3*e2*e3^~
#with the squares filled with 3 1's, 2 2's and 3 3's and one circle filled with 3.
def AllTableaux_e_Infinity(sequence_of_integers,entry_list):
    SOI = sequence_of_integers
    EL = entry_list
    Length = len(SOI)
    if SOI[0] > 0:
...
#Same as alltableaux e infinity but with s^0
def AllTableaux_e_Zero(sequence_of_integers,entry_list):
    SOI = sequence_of_integers
    EL = entry_list
    Length = len(SOI)
    if SOI[0] > 0:
...
def AllTableaux_h_Zero(sequence_of_integers):
    ...
    '''
#same as alltableaux e zero but with h's
def AllTableaux_h_Zero(sequence_of_integers,entry_list):
    T = AllTableaux_e_Zero(sequence_of_integers,entry_list)
    L = []
    for t in T:
        SP = np.transpose(t[0])
        L.append([SP,t[1]])
...
'''
#this gives all SYT of the superpartition La,Ls in relation to s^0
def AllFillingsZero(La,Ls,entry_list):
    ASF = AllSuperFillings(La,Ls)
    L = []
    for sp in ASF:
        AThZ = AllTableaux_h_Zero(sp,entry_list)
        for T in AThZ:
            if shape(T[0]) == (La,Ls):
                L.append(T)
    return L
...
#Basically all the transposes of AllTableaux e Infinity.
#(note on this is that it considers the SYT to be weakly increasing in rows and strictly in columns)
def AllTableaux_h_InfinityStar(sequence_of_integers,entry_list):
    T = AllTableaux_e_Infinity(sequence_of_integers,entry_list)
    L = []
    for t in T:
        SP = np.transpose(t[0])
        L.append([SP,t[1]])
...
'''
#All transposes of s^inf but weakly increasing in rows and strictly in colunms
def AllFillingsInfinityStar(La,Ls,entry_list):
    ASF = AllSuperFillings(La,Ls)
    L = []
    for sp in ASF:
        AThIS = AllTableaux_h_InfinityStar(sp,entry_list)
        for T in AThIS:
            if shape(T[0]) == (La,Ls):
                L.append(T)
    return L
...
#this takes in the content of the two-row notation and produces all possible pairs (P,Q) of syt that could correspond to it.
def RSK_possibilities(Q_row,P_row,entry_list_Q,entry_list_P,last_Q, last_P):
    possible_Q = AllTableaux_h_Zero(Q_row,entry_list_Q)
    possible_P = AllTableaux_e_Infinity(P_row,entry_list_P)
    output = []
    for i in possible_Q:
        for j in possible_P:
            if shape(i[0]) == shape(j[0]):
    ...
    for p in output:
        if (last_Q is None) or (isSubTableau(last_Q, p[0][0])):
            print(p[0][0])
            print(p[1][0])
            if p is not output[-1]:
                print("----------------")
            ...
            correct_output.append(p)
    print("Remaning possibilities", len(correct_output))
...
def arrayTohseq(trarray, size):
    ...
    for i in range(len(trarray[0])):
        ...
        # abs(trarray[0][i])-1 purely for indexing 1->0 purpose
        if trarray[0][i] < 0:
        ...
        num_collector_Q.add(abs(trarray[0][i]))
        num_collector_P.add(abs(trarray[1][i]))

    hseq_Q = [i*j for i,j in zip(collector_Q, sgn_Q) if i*j != 0]
    hseq_P = [i*j for i,j in zip(collector_P, sgn_P) if i*j != 0]

    hseq_Q = [i+1 if i<0 else i for i in hseq_Q]
    hseq_P = [i+1 if i<0 else i for i in hseq_P]

    num_collector_Q = sorted(list(num_collector_Q))
    num_collector_P = sorted(list(num_collector_P))


    #print(hseq_Q, hseq_P)
    return [hseq_Q, hseq_P, num_collector_Q, num_collector_P]
...
def RSK_path(trarrays, size):
    ...
    for i in range(len(trarrays[0])):
        ...
        print("--hseq:", hseqs_temp[:2], "correpsond to", hseqs_temp[2:])
        print("original two rows", trarrays_temp)

        if len(correct_output) != 0:
            last_Q = correct_output[0][0][0]
            last_P = correct_output[0][1][0]
        else:
            last_Q = None
            last_P = None
        correct_output = RSK_possibilities(hseqs_temp[0], hseqs_temp[1], hseqs_temp[2], hseqs_temp[3], last_Q, last_P)
        print("---------end of hseq------------")
